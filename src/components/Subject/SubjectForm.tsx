...
const SubjectForm = ({ element, changeId, changeObject, changeValueObject, deleteObject }: any) => {
			...
			<li key={element.id} className='list_row'>
				<form className='row_form'>
					<div className='row_form_texts'>
						<label>
							<input placeholder='имя *' name='title' value={data.title} onChange={handlerChangeValue} />
						</label>
					</div>
