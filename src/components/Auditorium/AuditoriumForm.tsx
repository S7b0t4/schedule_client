...
const AuditoriumForm = ({ element, changeId, changeObject, changeValueObject, deleteObject }: any) => {
			...
			<li key={element.id} className='list_row'>
				<form className='row_form'>
					<div className='row_form_texts'>
						<label>
							<input placeholder='номер кабинета *' name='number' value={data.number} onChange={handlerChangeValue} />
						</label>
					</div>
